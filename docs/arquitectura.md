...
### Reconexión rápida (delta por regiones)  Al caerse la conexión el cliente **conserva el mundo** que tenía y al volver pide el delta: `Unirse` lleva `TengoMundo` y el servidor responde `Unido` con `Delta = true` y `RegionesDelta` (cuántas regiones van detrás), seguidas de solo las `MundoRegion` que cambiaron mientras estaba fuera. Para saberlo, cada mundo guarda el número de cambio (`Cambios`) y la última vez que cambió cada región (`CambiosRegion`), y de cada jugador apunta al salir cuándo se fue, qué cambio vio y qué regiones tenía cargadas (`Salidas`). Si la ausencia supera `MinutosDeltaRapido` (5 por defecto), si cambió más de medio mundo o si no hay salida apuntada, se manda el mundo entero por regiones como siempre. Con el delta, el cliente olvida la región antes de aplicarla (una región repetida se ignora) y el resto del radio llega igual que al moverse. 
