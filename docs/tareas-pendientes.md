...
### Gameplay / red
- [ ] **Streaming por proximidad** *(en curso)*: Hecho: regiones de 64x64 columnas (`Mundo.LadoRegion`), ensamblador del cliente (`MundoRemoto`), protocolo con cabecera en `Unido` + una `MundoRegion` por mensaje y `MundoOlvida` para soltar, radio de carga del servidor (`GameServer.RadioRegiones`, 3x3 por defecto) con reenvio al cambiar de region, entrada con la region de los pies y colision que trata como solido lo pendiente. Falta: probarlo en partida de verdad (caminar y ver como se carga y se suelta el terreno), decidir si el radio se configura desde el archivo de ajustes y pulir el rendimiento (reenviar submallas al soltar puede dar tirones en Android).
