...
- [x] **Reconexion rapida (delta por regiones)** (v0.11.22, publicada con la release v0.11.23ía): hecho. Al volver, el cliente dice que aun tiene el mundo (`Unirse.TengoMundo`) y el servidor responde `Unido` con `Delta` y `RegionesDelta` y solo reenvía las regiones que cambiaron mientras estaba fuera; con la ausencia pasada (`MinutosDeltaRapido`), con medio mundo cambiado o sin salida apuntada se manda el mundo entero. Probado por socket (8 comprobaciones). La rama antigua `feature/delta-reconexion` (delta por bloques con `MundoDelta`) queda descartada: su lectura ambigua es el origen del desajuste de mensajes que rompía la suite. Falta verlo en partida de verdad.
