...
### Gameplay / red
- [ ] **Streaming por proximidad** *(en curso)*: Hecho: regiones de 64x64 columnas (`Mundo.LadoRegion`, `SerializarRegion`/`AplicarRegion`), ensamblador del cliente (`MundoRemoto`), protocolo con cabecera en `Unido` + una `MundoRegion` por mensaje (entrada, mundo privado y reconexion) y la primera parte del streaming: el servidor manda las regiones de la más cercana a la más lejana, el cliente entra con la región de los pies y el resto se dibuja según llega, tratando como sólida la zona pendiente. Falta: soltar (descargar) lo que queda lejos y afinar el radio de carga según se mueve el jugador.
