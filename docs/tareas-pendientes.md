...
### Gameplay / red
- [ ] **Streaming por proximidad** *(en curso)*: enviar al entrar solo los trozos cercanos al jugador y el resto conforme se mueve. Hecho: las regiones de 64x64 columnas (`Mundo.LadoRegion`, `SerializarRegion`/`AplicarRegion`), el ensamblador del cliente (`MundoRemoto`) y el protocolo, que ya manda la cabecera en `Unido` y una `MundoRegion` por mensaje (entrada, mundo privado y reconexion), todo con pruebas en la suite. Falta: que el servidor mande solo el radio cercano y lo amplíe al moverse, que el cliente arme el mundo sin esperar a tenerlo entero y que la colisión trate como sólida la zona aún no recibida.
