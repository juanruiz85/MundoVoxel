...
- [ ] Firma estable entre releases: hoy, sin los secretos, cada ejecución firma el APK con una clave de depuración nueva, así que Android rechaza la actualización en sitio (comprobado: las firmas de v0.11.19 y v0.11.23 no coinciden). Configurar los secretos del keystore de Android y del certificado del MSIX es lo que falta para que la promesa de "actualizar en sitio" se cumpla de verdad.
