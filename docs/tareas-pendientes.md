...
### Gameplay / red
- [ ] **Streaming por proximidad** *(en curso)*: Hecho: regiones de 64x64 columnas (`Mundo.LadoRegion`, `SerializarRegion`/`AplicarRegion`), ensamblador del cliente (`MundoRemoto`), protocolo con cabecera en `Unido` + una `MundoRegion` por mensaje (entrada, mundo privado y reconexion), el servidor manda las regiones de la más cercana a la más lejana, el cliente entra con la región de los pies, el resto se dibuja según llega y la zona pendiente es sólida; y la base para soltar lo lejano (`Mundo.OlvidarRegion` + `MundoRemoto.Olvidar`, 11 comprobaciones). Falta: que el servidor elija el radio, mande las regiones nuevas y avise de las que se sueltan, y afinar el radio según se mueve el jugador.
