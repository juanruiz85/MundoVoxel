...
- [x] Textos visibles fuera del idioma (v0.11.23): el bloque de inventario, crafteo, cofre y muerte estaba escrito a mano (14 textos en el XAML y 6 en el código) y también los títulos de la ventana y de las páginas. Ya salen de `es.lang` y la suite (252) comprueba que ningún texto visible se escriba a mano. De paso se retiran ocho `x:Name` que nadie usaba y se añaden 17 claves.
