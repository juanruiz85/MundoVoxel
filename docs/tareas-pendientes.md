...
- [x] **Cuentas por jugador** (v0.11.21, sin tag todavía): hechos los tres pasos. Paso 1, el almacén en Core (`Cuentas`: hash PBKDF2-SHA256 con sal, comparación en tiempo constante, validaciones, cambio de clave y persistencia en `cuentas.json`). Paso 2, el protocolo y el servidor (`Hola` con `Usuario` y `ClaveCuenta`, `CuentasObligatorias` y `RegistroAbierto`, el nombre autenticado con la grafía guardada y el tope de 5 intentos por minuto). Paso 3, el cliente (usuario y clave en el menú, recordados y reutilizados en la reconexión). Todo apagado por defecto. Queda probarlo a mano con el servidor en cuentas obligatorias.
