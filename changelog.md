...
- **Firma del APK con keystore propio (opcional)**: si están los secretos `ANDROID_KEYSTORE_B64`, `ANDROID_KEYSTORE_PASS`, `ANDROID_KEY_ALIAS` y `ANDROID_KEY_PASS`, el APK de release se firma con esa clave; si no están, se sigue firmando con la de depuración y el workflow lo avisa. La ruta de firma (y el nombre `-Signed` que espera el empaquetado) quedó probada en local con un keystore de prueba: `apksigner verify` confirma el firmante y los esquemas v1, v2 y v3.
...
- **El nombre del cofre no tenía texto**: la tabla de bloques del núcleo usa la clave `bloque.cofre`, que faltaba en el archivo de idioma (las otras 32 sí estaban). Se habría visto en crudo en cuanto se usara esa tabla. Ya está en su sitio, entre el cobre y la lava.
- **Comprobación nueva del idioma (la suite pasa de 237 a 242)**: la suite revisa que el archivo de idioma no tenga claves repetidas ni textos vacíos, que toda clave usada en el código exista y que no sobren textos de bloque. Es una comprobación entre ficheros: busca la raíz del repositorio subiendo desde la carpeta de compilación, y corre igual en CI y en local.
- **Restos en el protocolo**: de los 50 mensajes declarados, uno («MundoBorrado») no lo enviaba ni lo atendía nadie; los jugadores de un mundo borrado se enteran por el aviso `MUNDO_BORRADO`, que sí está traducido. Se retira, y la suite (que pasa a 243) vigila que todo mensaje declarado en `protocolo.cs` se use en algún otro fichero.
- **Limpieza de restos del diseño antiguo del control táctil**: el servicio de teclado ya solo tiene lo que se usa. El evento de soltar tecla y los dos métodos de simulación no los llamaba nadie (los botones táctiles de Android escriben el estado directamente en la vista), y con ellos se van siete constantes `Teclas.*` que solo existían para esa simulación. Los comentarios de las dos clases ya no dicen que Android pasa por ahí.
- **Textos de idioma sin usar**: cinco entradas de `es.lang` («Conectando.», «Descargando mundo.», «Tú», «Volar: {0}» y «(solo el creador)») no las pedía nadie. Se retiran, y la suite (que pasa a 244) vigila ya en los dos sentidos: toda clave usada existe en el archivo y todo texto del archivo se usa en alguna parte. Las claves `error.*` quedan exentas porque el cliente arma la clave con el código que manda el servidor.
- **Dos cosas que la documentación contaba mal**: la tabla del protocolo seguía nombrando un mensaje ya retirado y decía que borrar un mundo se autoriza por id de conexión, cuando el servidor lo comprueba por **nombre**: así el creador no pierde su mundo al reconectar.
- **`SensibilidadRaton` no hacía nada**: el ajuste estaba en `ajustes.config.json`, pero el cliente usaba un `1` escrito a mano como valor por defecto, así que el archivo no cambiaba nada. Ahora el ajuste es el valor por defecto (la sensibilidad que elijas en el menú sigue mandando y se guarda entre sesiones). Y la suite (que pasa a 245) vigila que todo campo de la configuración lo lea alguien: es la comprobación que habría cazado este y el de `MinutosDeltaRapido`.
- **El anti-cheat ya se ve en el archivo de ajustes**: los dos mandos de movimiento (`AntiCheatSaltoMax` y `AntiCheatVelocidadMax`) estaban documentados en el código pero no aparecían en el archivo de ejemplo, así que un servidor dedicado no tenía forma de descubrirlos. Ya están ahí, a `0` (desactivados).
- **El ladrillo ya se puede conseguir**: el bloque estaba definido entero (dureza, necesita pico, nombre, color) pero no salía de ningún sitio: ni receta, ni horno, ni generación del mundo, ni botín de mobs. Era el único bloque del juego imposible de obtener. Ahora se cuece en el horno igual que la arena se funde en cristal (tierra  ladrillo) y consume carbón, como las fundiciones. Y la suite (que pasa a 247) vigila las dos mitades: que todo bloque colocable salga del mundo, de una receta o del horno, y que todo objeto salga de una receta, del horno, del botín de un mob o de romper un bloque.
- **La grava tampoco salía de ningún sitio**: el bloque tenía nombre, color, icono y dureza de pala, pero el generador no lo colocaba en ninguna parte. Es el mismo caso que el cobre, que existía pero nunca se generaba. Ahora aparece en bolsas bajo tierra, como los minerales, y siempre reemplazando piedra: no toca la superficie, ni los árboles, ni los lagos. La comprobación nueva de la suite encontró este bloque y el ladrillo en la misma pasada.
- **Ayudantes que ya no llamaba nadie (y un guard que lo vigila)**: revisando el API del cliente aparecieron diez miembros públicos sin una sola llamada, declarados y documentados en su día y luego olvidados: el contador de mallas para diagnósticos, dos conversiones de color a mano que el renderizador ya hace con su paleta, el color de los mobs por tipo (se pintan con el diseño voxel, que trae sus propios colores), un "la cabeza está en lava" para el HUD que nunca llegó a usarse, "el trigo está maduro" (la madurez la resuelve la tabla de drops bloque a bloque), "esto es un bloque" (nadie compara a mano con la frontera de los 1000), "requiere pico" (esa regla vive en la tabla de drops) el toque de la hotbar y la propiedad pública del joystick táctil, que nadie leia (los campos del joystick si se usan). Retirados los diez. Y para que no vuelva a pasar, la suite comprueba ahora que todo miembro declarado con visibilidad lo mencione alguien, contando también el XAML: es la misma clase de comprobación que cazó el evento AlCancelar, pero general.
- **El release se cayó al publicar el MSIX, y ya se sabe por qué**: el primer intento de `v0.11.23` dejó el release sin publicar porque el paso que arma el paquete MSIX moría con `MSBUILD : error MSB1001: Unknown switch.` La causa estaba en una línea del workflow que parecía inocente: `$extraMsix = if (...) { @("-p:...") } else { @() }`. Cuando la condición se cumple y el array tiene **un solo** elemento, la asignación desde un `if` deja de ser un array y pasa a ser una cadena, y el splat de una cadena la expande carácter a carácter: a MSBuild le llegaban 38 argumentos y el primero era un `-` suelto. El job del zip se salvaba por casualidad, porque allí el `if` devuelve dos elementos y el array no colapsa. Reproducido y arreglado en local (los argumentos se montan ahora en un array explícito).
- **Los tipos pasan por el mismo rasero**: la comprobación que vigila los miembros declarados tiene ahora una hermana un nivel más arriba, para clases, records, structs, enums e interfaces. Un tipo que solo aparece en su propia declaración es el mismo resto de una función a medio hacer que un evento que nadie dispara. Se revisan los cuatro proyectos, el XAML y los ficheros de configuración cuentan como uso, y quedan exceptuados los dos puntos de entrada que arranca el sistema (la actividad de Android y el `Main` del servidor), porque quien los llama está fuera del repositorio. No encontró ninguno: el código estaba limpio en esta dimensión, y desde ahora hay quien lo vigila.
- **Un evento que nadie dispara ya no puede colarse**: el tercer hermano de la familia. Un evento suscrito y nunca lanzado fue un bug real de este proyecto (`AlCancelar`, que el cliente atendía y el servidor no disparaba nunca), y ni la comprobación de miembros ni la de tipos lo habrían visto, porque suscribirse ya cuenta como mencionar el nombre. La comprobación nueva exige un sitio que lo dispare de verdad. Se cazó a sí misma en la primera pasada, y las dos trampas quedaron documentadas en el código: leía `continue` como si fuera un evento porque su propia línea contiene la cadena `"event "`, y los cinco eventos que llevan un comentario al final de la línea no acababan en `;`. Los doce eventos del proyecto tienen disparo.
- **Lo que el cliente guarda en el disco, también**: cuarta hermana de la familia. El cliente apunta cuatro cosas en `Preferences` (la clave del servidor, la cuenta y su clave, y la sensibilidad del ratón) y la comprobación nueva exige que cada clave se lea **y** se escriba. Si un día alguien renombra una en un solo lado, la lectura se quedaría con el valor por defecto para siempre y no se enteraría nadie: no habría error, ni aviso, solo un ajuste que parece funcionar y no hace nada. Las cuatro claves están bien; desde ahora están vigiladas.
- **Lo que se ve, sale del idioma**: el bloque de inventario, crafteo, cofre y muerte estaba escrito a mano (14 textos en el XAML y 6 en el código) y no pasaba por el archivo de idioma, así que se habría quedado en español para siempre; lo mismo pasaba con los títulos de la ventana y de las dos páginas. Ahora los 23 literales salen de `es.lang` (17 claves nuevas) sin cambiar ni una letra de lo que se ve en pantalla, y de paso se retiran ocho nombres de XAML que no usaba nadie.
- **La quinta hermana, y la que más se cazó a sí misma**: la suite comprueba ya que ningún texto visible se escriba a mano fuera del idioma, mirando `Text`, `Title` y `Placeholder` tanto en el XAML como en el código. Deja fuera el propio proyecto de pruebas, que escribe sus mensajes a propósito. En la primera pasada confundía la clave con el texto (`Text = _idioma.O("clave")`) y sacó 72 avisos falsos; en la segunda, un valor con dos comillas seguidas (`Text=""`) le colaba la siguiente propiedad como si fuera el texto. Las dos trampas quedan comentadas en el código, como las del hermano anterior.
...
- **Cuentas por jugador (paso 2: entrar con cuenta)**: `Hola` lleva ahora `Usuario` y `ClaveCuenta`. Con `"CuentasObligatorias": true` en `ajustes.config.json` el servidor no saluda hasta validar la cuenta: si el nombre ya existe se comprueba la clave (PBKDF2, comparación en tiempo constante) y el jugador entra con la grafía guardada, de modo que el inventario y la propiedad de los mundos dejan de depender de cómo escriba su nombre cada vez. Si el nombre no existe y `"RegistroAbierto"` está activo, esa primera entrada da de alta la cuenta con la clave que trae (entrar por primera vez es registrarse) y `cuentas.json` se guarda al momento. Los fallos comparten el tope de 5 intentos por minuto con la clave del servidor y las de los mundos privados; el único aviso explícito es `CLAVE_CORTA` al darse de alta, y todo lo demás dice `CREDENCIALES` para no delatar qué nombres existen. Por defecto va apagado: los servidores que no pongan esas claves siguen exactamente igual.
- **Cuentas por jugador (paso 3: el cliente)**: el menú pide **usuario** y **clave de la cuenta** (dos campos nuevos, con su texto de idioma) y el cliente los recuerda igual que la clave del servidor; se mandan en el saludo y **también en la reconexión automática**, de modo que reconectar a un servidor con cuentas sigue funcionando. El mundo que se crea queda a nombre de la **cuenta**, no del nombre escrito a mano (la suite lo comprueba mandando un nombre distinto al de la cuenta y mirando de quién es el mundo en la lista).
- **Cuentas por jugador (paso 4: cambiar la clave)**: desde el menú de pausa se puede cambiar la clave de la cuenta (`CambiarClaveCuenta`). Hace falta la clave actual y solo lo atiende un servidor con cuentas obligatorias sobre una sesión ya autenticada; el servidor renueva la sal y guarda el archivo al momento, comparte el tope de 5 intentos por minuto con el resto de claves y avisa igual que al entrar (`CREDENCIALES`, `CLAVE_CORTA` y `MUCHOS_INTENTOS`). El cliente confirma con `ClaveCuentaCambiada`, guarda la clave nueva para la reconexión automática y la olvida si llega un aviso. Pruebas: 8 comprobaciones de integración por socket (con la vieja mal, con la nueva corta, con la vieja buena, que la nueva entra, que la vieja ya no vale y que en un servidor sin cuentas no cambia nada).

### Corregido

- **Textos internos con tildes doblemente codificadas**: el registro del servidor y varios comentarios y mensajes del código arrastraban tildes guardadas dos veces (se veían dos caracteres raros en lugar de la letra con tilde). Ahora esos textos van sin tildes, como el resto del código. También se ha corregido el mensaje de clave inválida del servidor, que seguía diciendo que la clave tiene 4 dígitos cuando son 6 desde la versión 0.11.1 (el cliente ya mostraba el texto bueno por su código de error).
## [0.11.20] - 2026-09-16
...
- **Radio de carga y aviso de olvido (`MundoOlvida`)**: al entrar se manda el mundo entero, de la region mas cercana a la mas lejana (asi el cliente puede entrar en cuanto tiene la region de los pies); a partir de ahí, el jugador solo recibe las regiones dentro del radio de carga (por defecto 3x3, `RadioRegiones` en `ajustes.config.json`) y, al cambiar de región, el servidor manda las que entran y avisa con `MundoOlvida` de las que se alejan. El cliente las descarga (bloques en aire + submallas vaciadas) sin dejar de tratar como solido el terreno que aun no ha llegado.
- **`RadioRegiones` en los ajustes**: el radio de carga se configura desde `ajustes.config.json` (por defecto 1 = 3x3 regiones = 192x192 columnas).
- **Mallas sucias (sin tirón al cargar o soltar una región)**: `MallasSucias` (Core) apunta las mallas que toca una región entera y el bucle de dibujo reconstruye unas pocas por frame. Antes se reconstruia paso a paso y cada paso reconstruia tambien las mallas vecinas (hasta cuatro veces la misma). Pruebas: la cola, el recorte a los bordes del mundo, el presupuesto por frame y que marcar dos veces no duplica el trabajo.
- **Pruebas**: la suite pasa a cubrir el olvido de regiones (contador, bloques en aire, repetir, fuera de rango y reaplicar), el calculo del radio de carga y el ciclo completo del streaming con un teletransporte (el servidor suelta una region lejana con `MundoOlvida` y al volver le manda las que entran en el radio).
